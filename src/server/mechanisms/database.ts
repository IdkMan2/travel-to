...
async function initializeClient(): Promise<IMongoClient | never> {
  assert.strictEqual(typeof process.env.MONGODB_URI, 'string', 'Env `MONGODB_URI` not defined.');
...
export async function getMongoDb(): Promise<Db | never> {
  assert.strictEqual(typeof process.env.DB_NAME, 'string', 'Env `DB_NAME` not defined.');
